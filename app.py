...
with tabs[0]:
    ...
    with st.container():
        user_input = st.text_input(
            "Your question",
            placeholder="e.g. Can an officer take my keys without an FIR?",
            key="chat_input",
            label_visibility="collapsed",
        )
        col1, col2 = st.columns([1, 5])
        send = col1.button("Send 🚦", key="chat_send")
        col2.button("Clear Chat 🗑️", key="chat_clear",
    ...
    if send and user_input.strip():
        ...
        with st.spinner("Thinking…"):
            reply = chat(st.session_state.api_messages)
...
with tabs[1]:
    ...
    if st.button("🔍 Validate Challan", key="chk_btn"):
...
with tabs[2]:
    ...
    with c2:
        ...
        dl_legal_fine = st.number_input("Correct Legal Fine (₹) — from Challan Validator", min_value=0, step=100, key="dl_lfine")
    ...
    if st.button("✉️ Generate Dispute Letter", key="dl_btn"):
        ...
        if missing:
            ...
            st.download_button("⬇️ Download as .txt", letter, file_name="dispute_letter.txt", mime="text/plain")

# ═════════════════════════════════════════════════════════════════════════════
# TAB 4 — FINE CALCULATOR
# ═════════════════════════════════════════════════════════════════════════════
with tabs[3]:
    ...
    if st.button("💰 Calculate Fine", key="fc_btn"):
...
with tabs[4]:
    ...
    if st.button("🧪 Calculate BAC", key="bac_btn"):
        try:
...
with tabs[5]:
    ...
    if st.button("📊 Check My Status", key="pp_btn"):
...
with tabs[6]:
    ...
    if st.button("🔍 Check All Documents", key="doc_btn"):
        all_ok = True
...
with tabs[7]:
    ...
    if st.button("⚖️ Compare States", key="sc_btn"):
...
with tabs[8]:
    ...
    if st.button("⚖️ Get Rights Advice", key="rights_btn"):
...
with tabs[9]:
    ...
    if st.button("💰 Get Overspeeding Fine", key="os_btn"):
